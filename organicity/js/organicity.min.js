/*! Organicity - v0.1.0 - 2015-06-18
 * http://www.organicity.eu
 * Copyright (c) 2015 Future Cities Catapult */
jQuery(document).ready(function(a){a("#owl-demo").owlCarousel({slideSpeed:300,paginationSpeed:400,singleItem:!0}),a('a[href^="#"]').on("click",function(b){var c=a(a(this).attr("href"));c.length&&(b.preventDefault(),a("html, body").animate({scrollTop:c.offset().top},1e3))}),a("#filter-menu-button").click(function(b){a("#filter-menu").toggleClass("active")}),a("a[href=#menuExpand]").click(function(b){a(".menu").toggleClass("menuOpen"),a(".menu-wrapper").toggleClass("active"),b.preventDefault()})});